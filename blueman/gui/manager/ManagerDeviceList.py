...
class ManagerDeviceList(DeviceList):
    ...
    def device_remove_event(self, device):
        ...
        self._faderhandlers.append(row_fader.connect("animation-finished", self.__on_fader_finished))

        row_fader.thaw()
        self.emit("device-selected", None, None)
        row_fader.animate(start=row_fader.get_state(), end=0.0, duration=400)

    def __on_fader_finished(self, fader):
        fader.disconnect(self._faderhandlers.pop(0))
        fader.freeze()
